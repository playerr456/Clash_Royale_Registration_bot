...
- `/api/register`:
  ...
  - после сохранения отправляет напоминание о подписке с кнопками для основного канала, канала CR Cup и `Проверить подписку`;
  - кнопка `Присоединится к чату турнира` показывается всегда; если `CHANNEL_URL_CR_CUP` не задана, бот попытается создать invite-ссылку по `CHANNEL_CHAT_ID_CR_CUP`.
...
2. Заполните переменные окружения:
   ...
   - `CHANNEL_URL_CR_CUP` (опционально, ссылка на второй канал для подписки)
   - `CHANNEL_CHAT_ID_CR_CUP` (опционально, `@username` или `-100...`; нужен для проверки второго канала)
