...
function deriveChannelChatId(channelUrl) {
  const match = String(channelUrl || "").match(/^https?:\/\/t\.me\/([A-Za-z0-9_]{5,})$/i);
...
]) || deriveChannelChatId(CHANNEL_URL);
const CHANNEL_URL_CR_CUP = normalizeUrl(
  readEnv([
    "CHANNEL_URL_CR_CUP",
    "TELEGRAM_CHANNEL_URL_CR_CUP"
  ])
);
const CHANNEL_CHAT_ID_CR_CUP = readEnv([
  "CHANNEL_CHAT_ID_CR_CUP",
  "TELEGRAM_CHANNEL_CHAT_ID_CR_CUP"
]) || deriveChannelChatId(CHANNEL_URL_CR_CUP);
